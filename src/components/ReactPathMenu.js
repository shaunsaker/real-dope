...
export default class ReactPathMenu extends React.Component {
  constructor(props) {
    super(props);
          ...
          <div>
            {interpolatedStyles.map(({ height, left, rotate, scale, top, width }, index) =>
              <div 
                ...
                <Link to={childButtonIcons[index].link}>
                  <Button
                    bsSize="sm"
                    bsStyle="default">
                    <span className="icon"><CustomIcon icon={childButtonIcons[index].icon} /></span>
                  </Button>
                </Link>
              </div>
            )}
